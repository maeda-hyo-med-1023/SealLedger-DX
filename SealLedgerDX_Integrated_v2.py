...
class SealLedgerDXApp:
    def __init__(self, root):
        self.root = root
        root.title("SealLedger DX - 統合自動化システム v2")
        root.geometry("980x780")
        root.minsize(900, 700)
        root.resizable(True, True)
        root.configure(bg="#f0f0f0")

        self.default_font = ("Meiryo", 10)
    ...
    # ============================================================
    # タブ1: フォーマット＆入力規則（省略：前回と同じ）
    # ============================================================
    def setup_tab1(self):
        parent = self.tab1
        pad = 10

        frame1 = ttk.LabelFrame(parent, text="①-1 Excelフォーマット複製（ページ追加）", padding=pad)
        ...
        ttk.Entry(row1, textvariable=self.fmt_template, width=40).pack(side="left", padx=5)
        ...
        ttk.Button(row1, text="▶ 複製実行", command=self.run_format_copy).pack(side="left", padx=10)

        frame2 = ttk.LabelFrame(parent, text="①-2 入力規則を自動セット", padding=pad)
        ...
        ttk.Entry(row2, textvariable=self.val_master, width=30).pack(side="left", padx=5)
        ...
        ttk.Entry(row3, textvariable=self.val_master_sheet, width=15).pack(side="left", padx=5)
        ttk.Label(row3, text="マスター列：").pack(side="left", padx=10)
        self.val_master_col = tk.StringVar(value="A")
        ttk.Entry(row3, textvariable=self.val_master_col, width=5).pack(side="left")

        row4 = ttk.Frame(frame2)
        row4.pack(fill="x", pady=2)
        ttk.Label(row4, text="対象Excel：").pack(side="left")
        self.val_target = tk.StringVar()
        ttk.Entry(row4, textvariable=self.val_target, width=30).pack(side="left", padx=5)
        ...
        ttk.Entry(row5, textvariable=self.val_target_sheet, width=15).pack(side="left", padx=5)
        ttk.Label(row5, text="設定列：").pack(side="left", padx=10)
        self.val_target_col = tk.StringVar(value="E")
        ttk.Entry(row5, textvariable=self.val_target_col, width=5).pack(side="left")
        ttk.Label(row5, text="開始行：").pack(side="left", padx=10)
        self.val_start_row = tk.IntVar(value=3)
        ttk.Entry(row5, textvariable=self.val_start_row, width=5).pack(side="left")
        ttk.Button(row5, text="▶ 入力規則セット", command=self.run_validation_set).pack(side="left", padx=10)
    ...
    def run_format_copy(self):
        ...
        try:
            ...
            for sheet in wb.sheetnames:
                if '校印使用台帳' in sheet:
                    template_sheet = wb[sheet]
                    break
            if template_sheet is None:
                template_sheet = wb[wb.sheetnames[0]]
            for i in range(1, pages + 1):
                new_sheet = wb.copy_worksheet(template_sheet)
                new_sheet.title = f"Page_{i}"
            wb.save(output)
            self.status_var.set(f"✅ 複製完了: {pages}ページ追加しました。")
    ...
    # ============================================================
    # タブ2: PDF変換＆フォルダ作成（簡略化）
    # ============================================================
    def setup_tab2(self):
        parent = self.tab2
        pad = 10

        frame1 = ttk.LabelFrame(parent, text="②-1 PDF → 画像変換", padding=pad)
        frame1.pack(fill="x", padx=pad, pady=pad)
        row1 = ttk.Frame(frame1)
        row1.pack(fill="x", pady=2)
        ttk.Label(row1, text="PDFファイル：").pack(side="left")
        self.pdf_path = tk.StringVar()
        ttk.Entry(row1, textvariable=self.pdf_path, width=35).pack(side="left", padx=5)
        ...
        ttk.Entry(row1, textvariable=self.pdf_output, width=25).pack(side="left", padx=5)
        ...
        ttk.Entry(row2, textvariable=self.pdf_dpi, width=6).pack(side="left", padx=5)
        ttk.Label(row2, text="ページ範囲（例: 1-10, 空欄=全ページ）：").pack(side="left", padx=10)
        self.pdf_range = tk.StringVar()
        ttk.Entry(row2, textvariable=self.pdf_range, width=15).pack(side="left")
        ttk.Button(row2, text="▶ 変換実行", command=self.run_pdf_convert).pack(side="left", padx=10)

        frame2 = ttk.LabelFrame(parent, text="②-2 フォルダ一括作成", padding=pad)
        frame2.pack(fill="x", padx=pad, pady=pad)
        row3 = ttk.Frame(frame2)
        row3.pack(fill="x", pady=2)
        ttk.Label(row3, text="ベース名：").pack(side="left")
        self.folder_base = tk.StringVar(value="page_")
        ttk.Entry(row3, textvariable=self.folder_base, width=15).pack(side="left", padx=5)
        ...
        ttk.Entry(row3, textvariable=self.folder_output, width=25).pack(side="left", padx=5)
        ttk.Button(row3, text="参照", command=lambda: self.browse_folder(self.folder_output)).pack(side="left")
        ttk.Button(row3, text="▶ フォルダ作成", command=self.run_folder_create).pack(side="left", padx=10)
    ...
    def setup_tab3(self):
        parent = self.tab3
        pad = 10
        frame = ttk.LabelFrame(parent, text="③ ファイル→同名フォルダ自動振り分け", padding=pad)
        ...
        ttk.Entry(row1, textvariable=self.router_base, width=15).pack(side="left", padx=5)

        row2 = ttk.Frame(frame)
        row2.pack(fill="x", pady=2)
        ttk.Label(row2, text="ソースフォルダ（ファイル）：").pack(side="left")
        self.router_source = tk.StringVar()
        ttk.Entry(row2, textvariable=self.router_source, width=35).pack(side="left", padx=5)
        ...
        ttk.Entry(row3, textvariable=self.router_target, width=35).pack(side="left", padx=5)
        ...
        ttk.Entry(row4, textvariable=self.router_suffix, width=15).pack(side="left", padx=5)
        ...
        ttk.Button(row4, text="▶ 一括振り分け", command=self.run_router).pack(side="left", padx=10)

        self.router_log = tk.Text(frame, height=8, font=("Consolas", 9), state="disabled", bg="#fafafa")
    ...
    def setup_tab4(self):
        ...
        common_frame = ttk.LabelFrame(parent, text="共通設定", padding=pad)
        ...
        ttk.Entry(row1, textvariable=self.t4_excel, width=40).pack(side="left", padx=5)
        ...
        ttk.Entry(row2, textvariable=self.t4_image_folder, width=40).pack(side="left", padx=5)
        ...
        ttk.Button(row3, text="▶ 次ページへ (+1)", command=self.t4_update_page).pack(side="left", padx=10)

        sub_notebook = ttk.Notebook(parent)
        sub_notebook.pack(fill="both", expand=True, padx=pad, pady=pad)

        self.sub1 = ttk.Frame(sub_notebook)
        sub_notebook.add(self.sub1, text="① 使途・文書番号")
        self.setup_t4_sub1()

        self.sub2 = ttk.Frame(sub_notebook)
        sub_notebook.add(self.sub2, text="② 日付・使用印 (D列)")
        self.setup_t4_sub2()

        self.sub3 = ttk.Frame(sub_notebook)
        ...
        self.t4_log = tk.Text(log_frame, height=6, font=("Consolas", 9), state="disabled", bg="#fafafa")
    ...
    def setup_t4_sub1(self):
        ...
        ttk.Entry(row, textvariable=self.t4_cut1_top, width=6).pack(side="left", padx=5)
        ttk.Label(row, text="下：").pack(side="left", padx=(10,5))
        self.t4_cut1_bottom = tk.DoubleVar(value=0.0)
        ttk.Entry(row, textvariable=self.t4_cut1_bottom, width=6).pack(side="left", padx=5)
        ttk.Label(row, text="左：").pack(side="left", padx=(10,5))
        self.t4_cut1_left = tk.DoubleVar(value=0.0)
        ttk.Entry(row, textvariable=self.t4_cut1_left, width=6).pack(side="left", padx=5)
        ttk.Label(row, text="右：").pack(side="left", padx=(10,5))
        self.t4_cut1_right = tk.DoubleVar(value=0.0)
        ttk.Entry(row, textvariable=self.t4_cut1_right, width=6).pack(side="left", padx=5)
        ttk.Button(row, text="✂ カット実行 (使途・文書番号)", command=self.t4_exec_cut_purpose).pack(side="left", padx=10)
    ...
    def setup_t4_sub2(self):
        parent = self.sub2
        pad = 10
        cut_frame = ttk.LabelFrame(parent, text="カット設定 (cm)", padding=pad)
        cut_frame.pack(fill="x", padx=pad, pady=pad)
        row = ttk.Frame(cut_frame)
        row.pack(fill="x", pady=2)
        ttk.Label(row, text="上：").pack(side="left", padx=(0,5))
        self.t4_cut2_top = tk.DoubleVar(value=0.0)
        ttk.Entry(row, textvariable=self.t4_cut2_top, width=6).pack(side="left", padx=5)
        ttk.Label(row, text="下：").pack(side="left", padx=(10,5))
        self.t4_cut2_bottom = tk.DoubleVar(value=0.0)
        ttk.Entry(row, textvariable=self.t4_cut2_bottom, width=6).pack(side="left", padx=5)
        ttk.Label(row, text="左：").pack(side="left", padx=(10,5))
        self.t4_cut2_left = tk.DoubleVar(value=0.0)
        ttk.Entry(row, textvariable=self.t4_cut2_left, width=6).pack(side="left", padx=5)
        ttk.Label(row, text="右：").pack(side="left", padx=(10,5))
        self.t4_cut2_right = tk.DoubleVar(value=0.0)
        ttk.Entry(row, textvariable=self.t4_cut2_right, width=6).pack(side="left", padx=5)
        ttk.Button(row, text="✂ カット実行 (日付・使用印)", command=self.t4_exec_cut_datestamp).pack(side="left", padx=10)

        resize_frame = ttk.LabelFrame(parent, text="リサイズ設定 (ピクセル)", padding=pad)
        resize_frame.pack(fill="x", padx=pad, pady=pad)
        row2 = ttk.Frame(resize_frame)
        row2.pack(fill="x", pady=2)
        ttk.Label(row2, text="幅：").pack(side="left", padx=(0,5))
        self.t4_resize_w2 = tk.IntVar(value=200)
        ttk.Entry(row2, textvariable=self.t4_resize_w2, width=6).pack(side="left", padx=5)
        ttk.Label(row2, text="高さ：").pack(side="left", padx=(10,5))
        self.t4_resize_h2 = tk.IntVar(value=200)
        ttk.Entry(row2, textvariable=self.t4_resize_h2, width=6).pack(side="left", padx=5)
        ttk.Button(row2, text="🔄 リサイズ実行", command=self.t4_exec_resize_datestamp).pack(side="left", padx=10)
        ttk.Button(row2, text="📌 D列に挿入", command=lambda: self.t4_insert_image("D", "datestamp")).pack(side="left", padx=10)
    ...
    def setup_t4_sub3(self):
        ...
        ttk.Entry(row, textvariable=self.t4_cut3_top, width=6).pack(side="left", padx=5)
        ttk.Label(row, text="下：").pack(side="left", padx=(10,5))
        self.t4_cut3_bottom = tk.DoubleVar(value=0.0)
        ttk.Entry(row, textvariable=self.t4_cut3_bottom, width=6).pack(side="left", padx=5)
        ttk.Label(row, text="左：").pack(side="left", padx=(10,5))
        self.t4_cut3_left = tk.DoubleVar(value=0.0)
        ttk.Entry(row, textvariable=self.t4_cut3_left, width=6).pack(side="left", padx=5)
        ttk.Label(row, text="右：").pack(side="left", padx=(10,5))
        self.t4_cut3_right = tk.DoubleVar(value=0.0)
        ttk.Entry(row, textvariable=self.t4_cut3_right, width=6).pack(side="left", padx=5)
        ttk.Button(row, text="✂ カット実行 (使用部署・取扱者)", command=self.t4_exec_cut_department).pack(side="left", padx=10)
        ...
        ttk.Entry(row2, textvariable=self.t4_resize_w3, width=6).pack(side="left", padx=5)
        ttk.Label(row2, text="高さ：").pack(side="left", padx=(10,5))
        self.t4_resize_h3 = tk.IntVar(value=200)
        ttk.Entry(row2, textvariable=self.t4_resize_h3, width=6).pack(side="left", padx=5)
        ttk.Button(row2, text="🔄 リサイズ実行", command=self.t4_exec_resize_department).pack(side="left", padx=10)
        ttk.Button(row2, text="📌 H列に挿入", command=lambda: self.t4_insert_image("H", "department")).pack(side="left", padx=10)
    ...
    def setup_tab5(self):
        ...
        # --- 設定エリア ---
        frame = ttk.LabelFrame(parent, text="OCR（画像→テキスト化）設定", padding=pad)
        frame.pack(fill="x", padx=pad, pady=pad)
        ...
        ttk.Entry(row1, textvariable=self.ocr_source, width=40).pack(side="left", padx=5)
        ...
        ttk.Entry(row2, textvariable=self.ocr_output, width=40).pack(side="left", padx=5)
        ...
        ttk.Entry(row3, textvariable=self.ocr_pattern, width=20).pack(side="left", padx=5)
        ttk.Label(row3, text="言語（例：jpn, eng）：").pack(side="left", padx=10)
        self.ocr_lang = tk.StringVar(value="jpn")
        ttk.Entry(row3, textvariable=self.ocr_lang, width=10).pack(side="left", padx=5)

        row4 = ttk.Frame(frame)
        row4.pack(fill="x", pady=2)
        ttk.Button(row4, text="▶ OCR実行（全画像をテキスト化）", command=self.run_ocr).pack(side="left", padx=5)
        ...
        log_frame.pack(fill="both", expand=True, padx=pad, pady=pad)
        self.ocr_log = tk.Text(log_frame, height=8, font=("Consolas", 9), state="disabled", bg="#fafafa")
    ...
    def run_ocr(self):
        ...
        for i, filename in enumerate(files, 1):
            ...
            try:
                ...
                text = text.strip()
                if text:
                    # .txt用：ファイル名を先頭に付けて縦に並べる
                    all_text_lines.append(f"--- {filename} ---")
                    all_text_lines.append(text)
                    all_text_lines.append("")  # 空行
                    # .md用
